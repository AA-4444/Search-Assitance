...
CORS(app, resources={r"/search": {"origins": [
    ...
    "https://your-frontend-railway-url.up.railway.app"  # Replace with your production frontend URL
]}})
...
def is_relevant_url(url, prompt_phrases):
    irrelevant_domains = [
        ...
        "wiktionary.org", "rezka.ag", "linguee.com", "bab.la", "reverso.net",
        "sinonim.org", "wordhippo.com", "microsoft.com", "romeo.com", "xnxx.com",
        "hometubeporn.com", "porn7.xxx", "fuckvideos.xxx"
    ]
...
def generate_search_queries(prompt, region="wt-wt"):
    ...
    prompt_phrases = [p.strip() for p in prompt.split(",")] or ["UI/UX design", "user interface", "user experience", "web design", "design inspiration"]
    web_queries = [prompt]
...
def serpapi_search(query, max_results=5, region="wt-wt"):
    ...
    if not api_key:
        logger.error("SERPAPI_KEY not set, falling back to DuckDuckGo")
        print("SERPAPI_KEY not set, falling back to DuckDuckGo")
        return "no_api_key", []
...
def duckduckgo_search(query, max_results=5, region="wt-wt"):
    ...
    logger.warning(f"Max retries reached for DuckDuckGo query {query}")
    print(f"Max retries reached for DuckDuckGo query {query}")
    return []

# Telegram search (stubbed since telegram=False)
def telegram_search(queries, prompt_phrases):
    logger.info("Telegram search skipped (telegram=False)")
    print("Telegram search skipped (telegram=False)")
    return []
...
def search_and_scrape_websites(queries, prompt_phrases, max_results=5, region="wt-wt"):
    ...
    for i, query in enumerate(queries, 1):
        logger.info(f"Processing web query {i}/{len(queries)}: {query}")
        print(f"Processing web query {i}/{len(queries)}: {query}")
        try:
    ...
    for i, url in enumerate(urls, 1):
        ...
        try:
            ...
            name_selectors = [
                "h1[class*='brand'], h1[class*='partner'], .company-name, .brand-name, .site-title, .logo-text, title, h1, .header-title, h2, .title",
                "meta[property='og:site_name']",
                "meta[name='application-name']"
            ]
            description_selectors = [
                ".description, .about, .content, .intro, div[class*='about'], section[class*='program'], p[class*='description'], meta[name='description'], div[class*='overview'], p",
                "meta[property='og:description']",
                "div[class*='intro'], section[class*='intro']"
            ]
            country_selectors = [
                ".location, .country, .address, .footer-address, div[class*='location'], div[class*='address'], footer, .contact-info",
                "meta[name='geo.country']",
                "span[class*='country'], p[class*='address']"
            ]
            ...
            for selector in name_selectors:
                ...
                if element:
                    name = clean_description(element.get("content") if element.get("content") else element.text)
                    if len(name) > 5:
                        break
            name = name or "N/A"
    ...
    try:
        with sqlite3.connect("/tmp/search_results.db") as conn:
            ...
            if not rows:
                ...
                return
            with open("/tmp/search_results.csv", "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(["ID", "Name", "Website", "Description", "Specialization", "Country", "Source", "Status", "Suitability", "Score"])
                writer.writerows(rows)
            logger.info("CSV file created: /tmp/search_results.csv")
            print("CSV file created: /tmp/search_results.csv")
    except Exception as e:
    ...
    try:
        with sqlite3.connect("/tmp/search_results.db") as conn:
            ...
            if not results:
                ...
                with open("/tmp/search_results.txt", "w", encoding="utf-8") as f:
            ...
            with open("/tmp/search_results.txt", "w", encoding="utf-8") as f:
                ...
                for row in results:
                    ...
                    f.write("\n" + "-"*50 + "\n")
            logger.info("TXT file created: /tmp/search_results.txt")
            print("TXT file created: /tmp/search_results.txt")
    except Exception as e:
...
def search():
    try:
        ...
        try:
            ...
            print(f"Failed to save results to files: {e}")
        
        response = {
            "results": all_results,
            "telegram_enabled": use_telegram,
            "region": region,
            "message": "No results found, possibly due to scraping failures or no matching content" if not all_results else "Search completed successfully"
        }
...
def download_file(filetype):
    ...
    filename = f"/tmp/search_results.{filetype}"
    try:
