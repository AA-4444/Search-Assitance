...
def clean_text(text: str) -> str:
    ...
    t = t.strip(" \n\t\r\"'`")
    while t and (t[0] in "[{" and t[-1] in "]}"): t = t[1:-1].strip()
    return t

def clean_description(description):
    if not description: return "N/A"
...
REGION_MAP = { "wt-wt":{"hl":"en","gl":"us"}, "us-en":{"hl":"en","gl":"us"}, "uk-en":{"hl":"en","gl":"gb"},
"de-de":{"hl":"de","gl":"de"}, "fr-fr":{"hl":"fr","gl":"fr"}, "ru-ru":{"hl":"ru","gl":"ru"},
"ua-ua":{"hl":"uk","gl":"ua"}, "kz-ru":{"hl":"ru","gl":"kz"}, "kz-kk":{"hl":"kk","gl":"kz"},
"by-ru":{"hl":"ru","gl":"by"}, "uz-ru":{"hl":"ru","gl":"uz"}, "az-ru":{"hl":"ru","gl":"az"},
"kg-ru":{"hl":"ru","gl":"kg"}, "am-ru":{"hl":"ru","gl":"am"}, "ge-ka":{"hl":"ka","gl":"ge"},
"pl-pl":{"hl":"pl","gl":"pl"}, "es-es":{"hl":"es","gl":"es"}, "it-it":{"hl":"it","gl":"it"},
"nl-nl":{"hl":"nl","gl":"nl"}, "se-sv":{"hl":"sv","gl":"se"}, "no-no":{"hl":"no","gl":"no"},
"fi-fi":{"hl":"fi","gl":"fi"}, "cz-cs":{"hl":"cs","gl":"cz"}, "sk-sk":{"hl":"sk","gl":"sk"},
"ro-ro":{"hl":"ro","gl":"ro"}, "hu-hu":{"hl":"hu","gl":"hu"}, "ch-de":{"hl":"de","gl":"ch"},
"tr-tr":{"hl":"tr","gl":"tr"}, "ae-en":{"hl":"en","gl":"ae"}, "in-en":{"hl":"en","gl":"in"},
"id-id":{"hl":"id","gl":"id"}, "ph-en":{"hl":"en","gl":"ph"}, "sg-en":{"hl":"en","gl":"sg"},
"my-ms":{"hl":"ms","gl":"my"}, "th-th":{"hl":"th","gl":"th"}, "vn-vi":{"hl":"vi","gl":"vn"},
"jp-ja":{"hl":"ja","gl":"jp"}, "kr-ko":{"hl":"ko","gl":"kr"}, "cn-zh":{"hl":"zh","gl":"cn"},
"ca-en":{"hl":"en","gl":"ca"}, "br-pt":{"hl":"pt","gl":"br"}, "mx-es":{"hl":"es","gl":"mx"},
"au-en":{"hl":"en","gl":"au"}, "nz-en":{"hl":"en","gl":"nz"} }
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            c.execute("""CREATE TABLE IF NOT EXISTS interactions (
                id TEXT PRIMARY KEY, query_id TEXT, url TEXT, domain TEXT,
                action TEXT, weight REAL, created_at TEXT)""")
...
# --- helpers for domain matching (subdomains included) ---
def domain_matches_any(dom: str, names: set) -> bool:
    if not dom: return False
    d = dom.lower().lstrip(".")
    for n in names:
        n = str(n).lower().lstrip(".")
...
def is_bad_domain(dom: str) -> bool:
    if not dom: return False
    d = dom.lower().lstrip(".")
    if d == "google.com" or d.endswith(".google.com"): return True
    if d.endswith("wikipedia.org"): return True
    return domain_matches_any(d, BAD_DOMAINS)
...
def is_knowledge_domain(dom: str) -> bool:
    return domain_matches_any(dom, KNOWLEDGE_DOMAINS)

SPORTS_TRASH_TOKENS = {"футбол","теннис","биатлон","хокей","баскетбол","волейбол","снукер",
...
INTENT_AFFILIATE = {"affiliate","аффилиат","аффилиэйт","партнерка","партнёрка","партнерская программа",
    "партнёрская программа","реферальная","referral","cpa","игемблинг","игейминг","igaming",
    "casino affiliate","affiliate network","партнеры казино","партнёры казино","аффилейт","афилейт","аффилированный"}
INTENT_LEARN = {"что такое","what is","определение","definition","гайд","guide","обзор","overview","курс","course","как работает","how to"}
CASINO_TOKENS = {"казино","casino","igaming","гемблинг","игемблинг","игейминг","беттинг","ставки","bookmaker","sportsbook"}
...
AGENCY_TOKENS_TEXT = {"agency","агентство","услуги","services","clients","клиенты","cases","кейсы",
    "program management","affiliate management","opm","partner program","igaming","casino affiliate"}
AGENCY_GATE_TOKENS = {"affiliate","affiliates","partner","partners","opm","igaming","casino"}
...
def looks_like_agency(text: str, url: str) -> bool:
    ...
    gate = any(tok in u or tok in t for tok in AGENCY_GATE_TOKENS)
    return gate and (url_hit or text_hit)
...
def domain_penalty(domain: str) -> float:
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            if cnt >= NEG_BAD_THRESHOLD: return NEG_BAD_SCORE_PENALTY
...
def history_boosts(intent: Dict[str,bool], region: str) -> Dict[str, float]:
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            c = conn.cursor()
            c.execute("""SELECT domain, COUNT(*) as cnt FROM results_history
                         WHERE intent_json=? AND region=? GROUP BY domain HAVING cnt>=2""",
...
# СУЖАЕМ сигналы в URL
COMPANY_URL_TOKENS = ["affiliate","affiliates","partner","partners","network","program","agency","opm"]
def is_relevant_url(url, prompt_phrases):
    if is_bad_domain(domain_of(url)): return False
    u = (url or "").lower()
    return any(tok in u for tok in COMPANY_URL_TOKENS)
...
def apply_geo_bias(queries: List[str], region: str) -> List[str]:
    ...
    for i,q in enumerate(queries):
        if i==0: biased.append(f"{q} {' '.join(tokens[:2])}".strip())
        elif i==1: biased.append(f"{q} site:.{tld}")
        else: biased.append(q)
    return biased

def geo_score_boost(url: str, text: str, region: str) -> float:
    hint = GEO_HINTS.get(region)
    if not hint: return 0.0
    boost = 0.0
    dom = domain_of(url)
    if dom.endswith(f".{hint['tld']}"): boost += 0.35
    t = (text or "").lower()
    if any(tok.lower() in t for tok in hint["tokens"]): boost += 0.15
...
def rank_result(description: str, prompt_phrases: List[str], url: str=None, region: str=None, boosts: Dict[str,float]=None) -> float:
    ...
    for w in words:
        if w in d: score += 0.25 if len(w)>6 else 0.15
    for p in prompt_phrases:
        if p.lower() in d: score += 0.3

    if url:
        uu=url.lower()
        if any(tok in uu for tok in COMPANY_URL_TOKENS): score += 0.25
        if looks_like_agency(d, uu): score += 0.35
        if boosts: score += boosts.get(domain_of(url), 0.0)
        pen = domain_penalty(domain_of(url))
        if pen: score -= pen

    if url and looks_like_blog(url, d): score -= 0.25
    if looks_like_listing(d, url or ""): score -= 0.5
    if looks_like_job_board(d, url or ""): score -= 0.7
    if looks_like_casino_operator(d, url or ""): score -= 0.8
    if looks_like_sports_garbage(d): score = min(score, 0.1)

    if url and region: score += geo_score_boost(url, d, region)
...
def gemini_expand_queries(user_prompt: str, region: str, intent: Dict[str,bool]) -> Tuple[List[str], List[str]]:
    ...
    system_instruction=("Ты помощник для поиска. Преобразуй запрос пользователя в массив из 8–12 КОРОТКИХ поисковых запросов по теме. "
    ...
    cached=cache_get(cache_key,12)
    if cached: return cached[:12], cached[:12]
    ...
    r=requests.post(url,params=params,json=body,timeout=20); r.raise_for_status()
    data=r.json()
    text=""
    try:
...
def fallback_expand_queries(user_prompt: str, region: str, intent: Dict[str,bool]) -> Tuple[List[str], List[str]]:
    ...
    t=base.lower(); cyr=is_cyrillic(t)
    if intent.get("affiliate"):
        en_boost=["affiliate programs","partner program application","affiliate marketing","referral program",
                  "CPA affiliate","best affiliate networks","affiliate agency","affiliate management agency"]
        if intent.get("casino"):
            en_boost+=["casino affiliate programs","iGaming affiliate agency","casino affiliate marketing agency",
            ...
            ru_boost=["аффилейт агентство казино","маркетинговое агентство iGaming","партнерская программа казино",
        ...
        queries+=[f"{base} официальный сайт",f"{base} каталог",f"{base} список",f"{base} компании",
    ...
    if cyr and intent.get("affiliate") and intent.get("casino"):
        queries+=["affiliate marketing companies for casino","casino marketing affiliate agencies",
...
NEGATIVE_SITES_FOR_BUSINESS=["site:wikipedia.org","site:en.wikipedia.org","site:ru.wikipedia.org",
...
NEGATIVE_INURL_FOR_BLOGS=["inurl:blog","inurl:news","inurl:guide","inurl:guides","inurl:insights","inurl:academy",
...
NEGATIVE_INTITLE_FOR_BLOGS=['intitle:"what is"','intitle:"how to"','intitle:"guide"','intitle:"overview"','intitle:"tips"']
NEGATIVE_SITES_CASINO=["site:askgamblers.com","site:casino.org","site:bonusfinder.com","site:gambling.com",
...
# ВАЖНО: не вырезаем '-казино'/'-casino'
NEGATIVE_TOKENS_CASINO_STRICT=["-слоты","-бонус","-игровые","-игровых","-депозит","-вывод","-slots","-bonus"]
...
def with_intent_filters(q: str, intent: Dict[str,bool]) -> str:
    ...
    if intent.get("business") and (intent.get("affiliate") and intent.get("casino")):
        ...
        negatives_casino=" ".join(NEGATIVE_TOKENS_CASINO_STRICT)
...
BLOG_URL_TOKENS=["/blog","/news","/article","/articles","/guides","/guide","/insights","/academy","/glossary",
    "/resources","/resource","/learn","/library","/case-study","/case-studies","/whitepaper","/press","/press-release"]
BLOG_TEXT_TOKENS_RU=["статья","советы","обзор","гайд","руководство","как начать","что такое","инсайты","новости",
    "пример","кейс","кейс-стади","исследование","белая книга","пресс-релиз","учебник","обучение","курс"]
BLOG_TEXT_TOKENS_EN=["article","guide","what is","how to","tips","overview","insights","news",
...
def looks_like_blog(url: str, text: str) -> bool:
    u=(url or "").lower()
    if any(tok in u for tok in BLOG_URL_TOKENS): return True
    t=(text or "").lower()
...
def duckduckgo_search(query, max_results=15, region="wt-wt", intent: Dict[str,bool]=None, force_ru_ddg=False):
    ...
    if data["count"] >= DAILY_REQUEST_LIMIT:
        logger.error("Daily request limit reached"); return []
...
def serpapi_search(query, max_results=15, region="wt-wt", intent: Dict[str,bool]=None):
    if not SERPAPI_API_KEY:
        logger.info("SERPAPI_API_KEY is not set; skipping SerpAPI"); return []
    q=with_intent_filters(query, intent or {"business":True})
...
def looks_like_definition_page(text: str, url: str, intent: Dict[str,bool]) -> bool:
    if not intent.get("business") or intent.get("learn"): return False
    t=(text or "").lower(); u=(url or "").lower()
    if any(k in t for k in ["что такое","what is","определение","definition","гайд","guide","курс","обзор","overview"]): return True
    dom=domain_of(u)
    if is_knowledge_domain(dom): return True
    return False

def is_probable_operator_domain(dom: str) -> bool:
    if not dom: return False
    d = dom.lower()
    return (("casino" in d) or ("slot" in d)) and not any(k in d for k in ["affiliate","affiliates","partner","partners"])
...
def search_and_scrape_websites(urls: List[str], prompt_phrases: List[str], region: str, intent: Dict[str,bool], boosts: Dict[str,float], query_id: str):
    ...
    results=[]; urls=list(dict.fromkeys(urls))[:80]
    for i, url in enumerate(urls, 1):
        logger.info(f"[{i}/{len(urls)}] Scraping: {url}")
        dom=domain_of(url)
        if is_bad_domain(dom) or domain_is_blocked(dom) or is_probable_operator_domain(dom):
            logger.info(f"Skip bad/blocked/operator domain: {url}"); continue
        proxy_attempts=0; proxies_used=[]; success=False
        while proxy_attempts <= MAX_PROXY_ATTEMPTS:
            proxy=None
            if proxy_attempts>0:
                proxy=get_proxy()
                if not proxy or proxy in proxies_used:
                    logger.warning(f"No more unique proxies for {url}"); break
                proxies_used.append(proxy)
            try:
                headers={"User-Agent": os.getenv("SCRAPER_UA","Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Safari/537.36")}
                resp=requests.get(url, headers=headers, timeout=20, proxies=proxy); resp.raise_for_status()
                ...
                if el: name=clean_description(getattr(el,"text","") or el)
                ...
                for sel in [
                    ...
                    if element:
                        ...
                        if len(description)>10: break

                country="N/A"
                el=soup.select_one(".location, .country, .address, .footer-address, div[class*='location'], div[class*='address'], footer, .contact-info")
                if el: country=clean_description(el.text)

                if looks_like_sports_garbage(description): success=True; break
                if looks_like_definition_page(description, url, intent): success=True; break
                if should_cut_blog(url, description, intent): success=True; break
                if looks_like_casino_operator(description, url): success=True; break
                if looks_like_listing(description, url): success=True; break
                if looks_like_job_board(description, url): success=True; break
                ...
                if is_relevant_url(url, prompt_phrases) or score>0.1:
                    row={"id":str(uuid.uuid4()),"name":name,"website":url,"description":description,
                         "country":country,"source":"Web","score":score}
                    results.append(row)
                ...
                if any(x in err for x in ["403","429","connection","timeout","ssl","remote disconnected"]):
                    ...
                    if proxy_attempts>MAX_PROXY_ATTEMPTS:
                        logger.warning(f"Max proxy attempts for {url}, skipping"); break
                else:
                    logger.error(f"Scrape failed for {url}: {e}, skipping"); break
        if not success:
...
def save_to_csv():
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            if not rows:
                logger.warning("No data to save to CSV"); return
...
def generate_search_queries(user_prompt: str, region="wt-wt") -> Tuple[List[str], List[str], str, Dict[str,bool]]:
    if region not in REGION_MAP:
        logger.warning(f"Invalid region {region}, defaulting to wt-wt"); region="wt-wt"
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            cursor.execute("""CREATE TABLE IF NOT EXISTS results (
                id TEXT PRIMARY KEY, name TEXT, website TEXT, description TEXT, specialization TEXT,
                country TEXT, source TEXT, status TEXT, suitability TEXT, score REAL)""")
            cursor.execute("DELETE FROM results"); conn.commit()
        ...
        for u in all_urls:
            ...
            if is_bad_domain(dom) or domain_is_blocked(dom) or is_probable_operator_domain(dom): continue
        ...
        for r in web_results:
            ...
            url_=r.get("website","")
            if is_bad_domain(dom) or domain_is_blocked(dom) or is_probable_operator_domain(dom): continue
            if any(url_.lower().endswith(t) for t in CASINO_TLDS): continue
            if looks_like_sports_garbage(txt): continue
            if intent.get("business") and not intent.get("learn") and is_knowledge_domain(dom): continue
            if should_cut_blog(url_, txt, intent): continue
            if looks_like_casino_operator(txt, url_): continue
            if looks_like_listing(txt, url_): continue
            if looks_like_job_board(txt, url_): continue
            filtered.append(r)
        all_results=prefer_country_results(filtered, region)
        all_results.sort(key=lambda x: x["score"], reverse=True)
        save_to_csv(); save_to_txt()
        return jsonify({"results": all_results, "region": region, "engine": engine, "query_id": query_id})
    except Exception as e:
        logger.error(f"API error: {e}")
        return jsonify({"error": str(e)}), 500

# --- FIX: разрешаем OPTIONS + алиас /api/feedback ---
